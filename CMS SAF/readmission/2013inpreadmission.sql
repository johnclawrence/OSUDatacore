with inpnextYear as(
select desy_sort_key, CLM_ADMSN_DT,CLAIM_NO from cms.[inp_claimsj_2013]
union select desy_sort_key, CLM_ADMSN_DT,CLAIM_NO from cms.[inp_claimsj_2014])
...
datediff("D",convert(date,convert(char(8),a.[CLM_THRU_DT])),min(convert(date,convert(char(8),c.CLM_ADMSN_DT)))) as readmission
	into dbo.[2013inp_readmission]
  FROM [CAT1].[cms].[inp_claimsj_2013] as a
