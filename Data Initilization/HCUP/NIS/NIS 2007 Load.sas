...
* "%trim(&filepath)NIS/NIS_2007/Data/NIS_2007   *;  
...
LIBNAME isilon ODBC NOPROMPT="Driver={SQL Server Native Client 11.0};Server=sql-ctlst-vp01\TP;Database=isilon;Trusted_Connection=yes;" SCHEMA = hcup;
            ...
            /*****************************************************************************
* SASload_NIS_2007_Hospital.SAS
* This program will load the 2007 NIS ASCII Hospital File into SAS.
*****************************************************************************/
...
DATA NIS_2007_Hospital; 
INFILE "%trim(&filepath)NIS/NIS_2007/Data/NIS_2007_Hospital.asc" LRECL = 207;                                                                                                                                                                                                    
...
INPUT 
      ...
      @182    YEAR                     N4PF.
      @186    HOSP_RNPCT               N3PF.
      @189    HOSP_RNFTEAPD            N4P1F.
      @193    HOSP_LPNFTEAPD           N4P1F.
      @197    HOSP_NAFTEAPD            N4P1F.
      @201    HOSP_OPSURGPCT           N3PF.
      @204    HOSP_MHSMEMBER           N2PF.
      @206    HOSP_MHSCLUSTER          N2PF.
      ;
RUN;
/*****************************************************************************
* SASload_NIS_2007_Dx_Pr_Grps.SAS
* This program will load the 2007 NIS ASCII Dx_Pr_Grps File into SAS.
*****************************************************************************/
...
*** Data Step to load the file ***;
DATA NIS_2007_Dx_Pr_Grps; 
INFILE "%trim(&filepath)NIS/NIS_2007/Data/NIS_2007_Dx_Pr_Grps.asc" LRECL = 154;                                                                                                                                                                                                    
...
RUN;
/*****************************************************************************
* SASload_NIS_2007_Core.SAS
* This program will load the 2007 NIS ASCII Core File into SAS.
*****************************************************************************/
...
*** Data Step to load the file ***;
DATA NIS_2007_Core; 
INFILE "%trim(&filepath)NIS/NIS_2007/Data/NIS_2007_Core.asc" LRECL = 508;                                                                                                                                                                                                    
...
INPUT 
      ...
      @279    NECODE                   N3PF.
      @282    NEOMAT                   N2PF.
      @284    NIS_STRATUM              N4PF.
      @288    NPR                      N2PF.
      @290    PAY1                     N2PF.
      @292    PAY1_X                   $CHAR10.
      @302    PAY2                     N2PF.
      @304    PAY2_X                   $CHAR10.
      @314    PL_NCHS2006              N3PF.
      @317    PR1                      $CHAR4.
      @321    PR2                      $CHAR4.
      @325    PR3                      $CHAR4.
      @329    PR4                      $CHAR4.
      ...
      @461    PRDAY14                  N3PF.
      @464    PRDAY15                  N3PF.
      @467    PointOfOriginUB04        $CHAR1.
      @468    PointOfOrigin_X          $CHAR8.
      @476    RACE                     N2PF.
      @478    TOTCHG                   N10PF.
      @488    TOTCHG_X                 N15P2F.
      @503    YEAR                     N4PF.
      @507    ZIPINC_QRTL              N2PF.
      ;
RUN;
/*****************************************************************************
* SASload_NIS_2007_Severity.SAS
* This program will load the 2007 NIS ASCII Severity File into SAS.
*****************************************************************************/
...
*** Data Step to load the file ***;
DATA NIS_2007_Severity; 
INFILE "%trim(&filepath)NIS/NIS_2007/Data/NIS_2007_Severity.asc" LRECL = 172;                                                                                                                                                                                                    
...
proc SQL;
create table isilon.NIS_2007_HOSPITAL
like work.NIS_2007_HOSPITAL;

proc SQL;
insert into isilon.NIS_2007_HOSPITAL
select * from work.NIS_2007_HOSPITAL;

proc SQL;
create table isilon.NIS_2007_CORE
like work.NIS_2007_CORE;

proc SQL;
insert into isilon.NIS_2007_CORE
select * from work.NIS_2007_CORE;

proc SQL;
create table isilon.NIS_2007_Dx_Pr_Grps
like work.NIS_2007_Dx_Pr_Grps;

proc SQL;
insert into isilon.NIS_2007_Dx_Pr_Grps
select * from work.NIS_2007_Dx_Pr_Grps;

proc SQL;
create table isilon.NIS_2007_SEVERITY
like work.NIS_2007_SEVERITY;

proc SQL;
insert into isilon.NIS_2007_SEVERITY
select * from work.NIS_2007_SEVERITY;



proc delete data= work.NIS_2007_HOSPITAL; run;
proc delete data= work.NIS_2007_CORE; run;
proc delete data= work.NIS_2007_Dx_Pr_Grps; run;
proc delete data= work.NIS_2007_SEVERITY; run;